...
A su vez, el estimar cuanto se debería comprar es una tarea que hoy en día, debido a los costos asociados, queda en manos del personal con mayor conocimiento del negocio. Inclusive, muchas veces recae directamente en el dueño, quien debe considerar no solo las ventas que ya ocurrieron, sino también cómo los factores externos, como los feriados, el clima y el momento del mes, podrían influir en las ventas de la próxima semana.
...
Formalmente, el aprendizaje supervisado parte de un conjunto de entrenamiento conformado por $n$ ejemplos etiquetados $\{(x_i, y_i)\}_{i=1}^{n}$, donde cada $x_i$ representa una entrada (también conocida como vector de características) y cada $y_i$ la salida o etiqueta correspondiente. Estos ejemplos son generados por una función desconocida $f(x)$, y el objetivo del algoritmo es encontrar una función aproximada $h(x)$, denominada \emph{hipótesis}, que generalice correctamente sobre datos no vistos.
...
Basándonos en \parencite{hyndman2018forecasting, james2013isl} se procede a definir las tres métricas principales para modelos de regresión.
...
Con el objetivo de sintetizar las principales características de las soluciones comerciales vigentes en Argentina y contrastarlas con la propuesta de SmartStocker, se elaboró la Tabla~\ref{tab:comparacion-funcionalidades}. En ella se ponen en relación funcionalidades clave vinculadas a la gestión del inventario, los reportes en tiempo real y, especialmente, la incorporación de modelos de Machine Learning.  
\begin{table}[htbp]
\centering
...
\hline
\textbf{Funcionalidad} & \textbf{Maxirest} & \textbf{Restô} & \textbf{SmartStocker} \\ \hline
Reportes en tiempo real          & \cmark & \cmark & \cmark \\ \hline
Control de inventario            & \cmark & \cmark & \cmark \\ \hline
Integración con plataformas      & \cmark & \cmark & \cmark \\ \hline
Facturación/gestión admin.       & \cmark & \cmark & \xmark \\ \hline
Predicción de demanda (ML)       & \xmark & \xmark & \cmark \\ \hline
Estimación dinámica de stock     & \xmark & \xmark & \cmark \\ \hline
Variables externas en modelos    & \xmark & \xmark & \cmark \\ \hline
Feedback de usuarios (ajuste)    & \xmark & \xmark & \cmark \\ \hline
Tablero de métricas predictivas  & \xmark & \xmark & \cmark \\ \hline
Alertas preventivas de stock     & \xmark & \xmark & \cmark \\ \hline
Enfoque exclusivo en gastronomía & \cmark & \cmark & \cmark \\ \hline
\end{tabular}
...
Ahora bien, la Matriz \emph{Eliminar-Reducir-Incrementar-Crear}, es la representación tabular del Marco de las Cuatro Acciones, y tiene como finalidad ayudar en la formación de las ideas para que la toma de decisiones tenga como objetivo la diferenciación y el bajo costo de manera integrada.
...
\subsection{Matriz ERIC}\label{sec:eric}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{images/matrizEric.jpeg}
    \caption{Matriz ERIC de SmartStocker.}
    {\textit{Fuente: Elaboración propia.}}
    \label{fig:eric}
\end{figure}

Las variables identificadas en la matriz ERIC (Figura~\ref{fig:eric}) se utilizan como insumo para construir la Curva de Valor (Figura~\ref{fig:curva}), con el objetivo de visualizar el posicionamiento de \emph{SmartStocker} en relación con sus competidores en el mercado.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{images/curvaValor.jpeg}
...
\subsection{Análisis del Estado del Arte}\label{sec:sintesis-estado}
...
Por otro lado, debe considerarse el análisis de la ventas como un factor fundamental para determinar la logística que deberá emplear la empresa para la reposición de inventario y así establecer su modelo de gestión. La predictibilidad se determina por elementos probables y no por factores aleatorios, ya que la toma de decisiones se hará en función de estadísticas fidedignas arrojadas por el sistema alimentado con datos actualizados proporcionados por el usuario.
...
Con el objetivo de comprender en profundidad las necesidades reales del sector gastronómico y validar los supuestos del proyecto, se llevó a cabo una fase de investigación con usuarios orientada a explorar sus necesidades, hábitos y percepciones. Esta etapa permitió recopilar información proveniente tanto de propietarios y administradores de locales gastronómicos como de consumidores finales, con el fin de identificar puntos de dolor, comportamientos y expectativas en torno a la gestión del inventario y la disponibilidad de productos.
\subsubsection{Entrevistas}\label{sec:entrevistas}

Durante la etapa de descubrimiento del proyecto, realizada en el año 2025, se consideró fundamental realizar entrevistas cualitativas con el fin de comprender a profundidad las múltiples dificultades que se enfrentan actualmente en la administración de negocios gastronómicos, buscando indagar tanto en las dinámicas como en los patrones diarios dentro del sector, especialmente en la gestión de inventarios y su impacto sobre dichos negocios.

Primeramente se entrevistó a Ulises Litterio, dueño de La Brava Burguer, la cual es una cadena gastronómica que consta de tres locales y 25 empleados. Se centra en la venta de hamburguesas y llevan a cabo todas las actividades correspondientes para el mantenimiento y desarrollo de calidad de las mismas que van desde la compra de materia prima e insumos, hasta la venta final de estas con opción de delivery propio. Ahora bien, el control del inventario se encuentra descentralizado, básicamente cada jefe de cocina realiza un conteo que se le informa al encargado. Dicha información es registrada en una planilla de Excel para su posterior revisión por parte del propietario. Además, todas las compras necesarias son realizadas de manera semanal, aunque pueden repetirse indistintamente si hay productos o ingredientes faltantes.

El cálculo de reposición de stock se basa en datos tales como las ventas de semanas anteriores, fechas clave, como feriados, y la intuición de los trabajadores sobre la demanda esperada. Sin embargo, gestionar el inventario de esta manera conlleva a una alta tasa de error tanto por sobrestock como por faltantes, lo cual genera desperdicios tanto a nivel económico como alimenticio. Además, Ulises destaca la dificultad de consolidar datos de ventas debido a que operan con múltiples plataformas de delivery como Rappi y PedidosYa, cada una con su propio sistema individual, y aunque ha considerado la opción de emplear un software como MaxiRest, ha terminado por descartar dicha idea debido a los costos elevados que tendría que pagar. 

En segundo lugar, se entrevistó al Sr. Oscar Campione, dueño de una rotisería familiar que opera con un único local. El personal que labora se encuentra compuesto por cuatro integrantes de la familia, exceptuando al repartidor. La venta del negocio despacha exclusivamente a través de delivery propio utilizando la plataforma PedidosYa. Además, el personal se encuentra compuesto por cuatro personas los cuales son todos integrantes de la familia, exceptuando al repartidor. 

Su modalidad de trabajo actualmente consiste en preparar y comercializar productos elaborados y ofrecidos en el mismo día, garantizando así la máxima frescura y calidad para el consumidor. Llevan manualmente un control de pedidos y ventas que contabilizan cada noche, y realizan el cálculo de reposición diariamente para poder comprar los insumos a la mañana siguiente y restituir solo el stock consumido. Esta estructura de negocio los expone a constantes errores que pueden afectar significativamente los costos para un local de tamaño pequeño, ya sea por falta de planificación, sobreestimación, pérdida de productos, ventas, falta de disponibilidad o exceso de producto perecederos, lo cual a su vez genera en la familia frustración y pérdida de fidelidad. 

Cabe resaltar que los dueños intentaron incorporar un software llamado “Delivery 5.0”. Sin embargo, este únicamente les permitía gestionar pedidos, dejando su mayor necesidad sin satisfacer, la predicción de ventas y la reposición de inventario. Por otro lado, el entrevistado señala que un software que pueda ayudarles a visualizar cuáles productos tienen mayor venta y cuánto se estima que vendieron en cierta cantidad de tiempo los ayudaría tanto a evitar el sobrestock como a generar promociones con base en datos reales.

\subsubsection{Encuesta}\label{sec:encuesta}

Con el objetivo de comprender los hábitos de consumo en locales gastronómicos y la percepción de los clientes respecto a la disponibilidad de los productos, se realizó una encuesta a más de 150 personas. El propósito de la misma fue identificar cómo la falta de stock y la planificación de inventario influyen directamente en la satisfacción y fidelización de los consumidores, un factor determinante en el mundo gastronómico. 
...
Estos resultados, junto con el cuestionario completo aplicado, pueden consultarse en el Anexo~\ref{anexo:encuesta}, donde se detalla la encuesta utilizada y la totalidad de sus preguntas.


\subsubsection{Análisis del User Research}\label{sec:sintesis-user-research}
