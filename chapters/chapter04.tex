...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{images/DiagramaCasosDeUsoTesis.png}
...
El imagotipo integra un isotipo geométrico que sugiere \textit{contenedor/caja} (inventario) y una flecha ascendente (proyección/abastecimiento), combinado con el logotipo. El isotipo admite el uso independiente de tamaños reducidos, mientras que el imagotipo es la versión preferente para cabeceras, presentaciones y piezas institucionales.
...
A continuación se presentan las principales pantallas de la aplicación SmartStocker, ordenadas según el flujo de interacción del usuario. Estas capturas abarcan: la experiencia inicial (Figuras~\ref{fig:ux-landing1}--\ref{fig:ux-login}); el panel principal (Figuras~\ref{fig:ux-dashboard1}--\ref{fig:ux-dashboard3}); la gestión de datos (Figuras~\ref{fig:ux-ingredientes}--\ref{fig:ux-datos-historicos}); y las funcionalidades de predicción, retroalimentación y alertas (Figuras~\ref{fig:ux-predicciones}--\ref{fig:ux-alertas}).
\begin{figure}[htbp]
    \centering
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{images/cargaMasiva.png}
    \caption{Pantalla para carga masiva de ingredientes}
    {\textit{Fuente: Elaboración propia.}}
    \label{fig:ux-carga-masiva}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{images/datosHistoricos.png}
    \caption{Pantalla para carga de datos históricos de ventas.}
    {\textit{Fuente: Elaboración propia.}}
    \label{fig:ux-datos-historicos}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{images/feedback.png}
    \caption{Pantalla de retroalimentación de predicciones por parte del usuario.}
    {\textit{Fuente: Elaboración propia.}}
    \label{fig:ux-feedback}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{images/alertas.png}
    \caption{Pantalla de gestión y visualización de alertas de stock.}
    {\textit{Fuente: Elaboración propia.}}
    \label{fig:ux-alertas}
\end{figure}

\section{Funcionalidades}\label{sec:funcionalidades}

El presente apartado describe las funcionalidades principales que conforman la plataforma SmartStocker. Cada una de ellas fue diseñada con el objetivo de optimizar la gestión operativa de los locales gastronómicos, integrando procesos de predicción, control y automatización. 
...
\begin{itemize}
    ...
    \item \textbf{Amazon SageMaker:} Plataforma integral de machine learning que permite construir, entrenar y desplegar modelos de Machine Learning. Al ser la solución nativa de AWS para este tipo de aplicaciones, proporciona la capacidad de ejecutar los procesos dentro de su propio entorno, lo que garantiza una mayor disponibilidad de recursos en comparación con otras alternativas, como EC2 o Lambda.
...
Para la capa de almacenamiento, se optaron por dos soluciones. Primero, DocumentDB como base de datos NoSQL, seleccionado debido a la naturaleza no estructurada de los datos a utilizar, la cual brinda la posibilidad de modificar el schema con facilidad, además de ser una base serverless escalable. Asimismo, S3, para contener la información en formato \verb|csv| requerida para los entrenamientos del modelo, y para almacenar los archivos correspondientes al modelo entrenado en si.
...
Por último, se decidió usar AWS Sagemaker en el pipeline de ML, puesto que simplifica el entrenamiento del modelo, requiriendo suministrarle los dataset de entrenamiento y validación a utilizar, y permite disponibilizar el modelo para ser usado para predicciones mediante endpoints, lo que brinda una consulta rápida y de fácil implementación.
...
Para el diseño de la base de datos, se optó por utilizar DocumentDB, una de las alternativas NoSQL brindadas por AWS, debido a la flexibilidad y capacidades de escalado automático que esta provee, siendo la flexibilidad del schema algo crítico dada la necesidad de agregar campos adicionales conforme el proceso de ETL o entrenamiento del modelo de ML lo requieran.
...
La entidad CorreccionPrediccion guarda el feedback explícito del usuario sobre los valores predichos por el modelo. Cada registro asocia un itemMenuId y un userId con la predicción original, el valor corregido, la fecha y el turno, permitiendo incluir el feedback del usuario en la predicción.
...
El mismo se encuentra implementado mediante el uso de AWS EventBridge, a fin de programar entrenamientos periódicos de forma semanal. Este dispara un evento que activa la primera Lambda, dedicada a consolidar los datos de la semana, y unificarlos con el dataset histórico ya existente. Como parte de esta, también se obtendrán los datos correspondientes a feriados y aspectos climáticos.
...
descripción\_clima & Descripción textual del clima \\ \hline
\end{tabular}
...
Como parte del entrenamiento, se decidió usar la función de pérdida Tweedie, debido a su capacidad para modelar variables continuas no negativas con distribución asimétrica y presencia de ceros, como ocurre en las ventas diarias de productos gastronómicos, donde es una ocurrencia normal que ciertos productos no sean vendidos en determinados días. Esto permite que el modelo se adapte mejor a estos patrones, mejorando la precisión de las predicciones.

A continuación, se muestra un gráfico donde se visualiza la importancia de las diez features más relevantes, calculado usando la función PredictionValuesChange de CatBoost, la cual mide el impacto de cada feature en las predicciones del modelo al observar los cambios en los valores predichos cuando se modifica cada feature individualmente.
...
Usando estos hiperparámetros, se logran los siguientes resultados al evaluar el modelo:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/resultados.png}
    \caption{Resultados del modelo.}
    {\textit{Fuente: Elaboración propia.}}
    \label{fig:resultados-modelo}
\end{figure}

Podemos ver que tanto el RMSE como el MAE, medidos en unidades de venta, presentan valores de error aceptables en el contexto gastronómico, lo que indica que el modelo es apto para la tarea de predicción de ventas enfocada a la gestión de inventario.

\subsection{Inclusión del Feedback de usuario}\label{sec:inclusion-feedback-usuario}

A fin de incluir el feedback del usuario en las predicciones, se decidió implementar una lógica que ajuste la salida del modelo. Para ello, cada vez que el usuario informe que una predicción no es precisa en base a su experiencia, podrá informar el valor de ventas que él considera adecuado, ajustando esto el cálculo de stock requerido. Cuando esto ocurra de forma reiterada para una cierta combinación producto/turno dentro de un plazo de tiempo, se utilicen los valores corregidos para calcular un valor de ajuste, teniendo más peso las correcciones recientes, y se aplicará a la predicción obtenida del modelo. 

Esto permite realizar ajustes sobre posibles tendencias que están ocurriendo con mayor rapidez de la que el modelo es capaz de captar, sin modificar su proceso de entrenamiento. Eventualmente, si estos cambios en las tendencias llegan a consolidarse como el comportamiento normal, el modelo los aprenderá de forma natural a medida que acceda a nueva información.
...
\section{Consideraciones Legales}\label{sec:consideraciones-legales}

En relación a los aspectos legales, SmartStocker cumple con las prácticas estándares de la industria en cuanto a privacidad y seguridad de datos, usando cifrados automáticos disponibilizados por AWS para los datos de ventas de cada usuario, y HTTPS para cifrado de los datos en tránsito, tanto entre los componentes del sistema, como con las distintas plataformas de delivery. A su vez, el sistema está diseñado de forma tal que los datos se encuentran aislados a nivel usuario, lo que evita fugas de información entre los distintos usuarios, y permite que, en caso de que un usuario decida eliminar su cuenta, todos sus datos sean eliminados de forma completa del sistema.

En lo que respecta a las plataformas de delivery, el acceso tanto a PedidosYa como a Rappi se realiza mediante sus APIs oficiales, las cuales cuentan con términos y condiciones claras en cuanto al uso de los datos, cuyos aspectos más relevantes para el diseño de SmartStocker son las formas de autenticación y formatos de respuesta a usar, a fin de cumplir con ellos y mantener la plataforma operativa. El acceso a los datos de cada usuario se realiza usando para ambos casos los medios de asociación a la plataforma provistos por cada una, siendo estos PedidosYa Partners (y la correspondiente aprobación del usuario para la integración dentro de la plataforma) y las credenciales propias de la tienda en Rappi, respectivamente.