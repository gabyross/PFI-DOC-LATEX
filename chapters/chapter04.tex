...
\begin{itemize}
    ...
    \item \textbf{Amazon SageMaker:} Plataforma integral de machine learning que permite construir, entrenar y desplegar modelos de Machine Learning. Como la solución nativa de AWS para este tipo de aplicaciones, nos provee la capacidad de ejecutar nuestros procesos dentro de su entorno, lo que permite contar con una mayor disponibilidad de recursos en comparación con otras alternativas, como EC2 o Lambdas.
...
\begin{itemize}
    \item Factores cronologicos, siendo estos el día, semana y mes de la venta, como también el turno de trabajo en el cual fue realizada, y si la fecha corresponde a un feriado o no.
...
\begin{itemize}
    \item \textbf{cantidad}: Cantidad vendida de un producto, para una determinada fecha, siendo esta la variable a predecir por el modelo.
    \item \textbf{fecha}: Fecha de la venta en formato DD/MM/YYYY (usado para agrupación y features temporales)
    \item \textbf{producto}: Código del producto/ítem del menú.
    \item \textbf{turno}: Turno de venta.
\end{itemize}

A partir de estos datos, se generan los siguientes tipos de features adicionales:

\textbf{Features Temporales:} Representan los aspectos cronologicos de una venta. A fin de capturar la naturaleza ciclica de estos datos, se opto por representar a las variables de dia, semana y mes mediante el resultado de aplicar las funciones trigonométricas seno y coseno a sus correspondientes valores numericos, de esta forma, se logra que el modelo pueda captar la continuidad entre el final y el comienzo de cada ciclo (por ejemplo, entre el dia 31 y el dia 1).
...
\textbf{Features Temporales:}
\begin{itemize}
    \item \textbf{semana}: Semana del mes.
    \item \textbf{semana\_cos}: Coseno del valor numerico de la semana.
    \item \textbf{semana\_sin}: Seno del valor numerico de la semana.
    \item \textbf{mes}: Número del mes (1-12)
    \item \textbf{mes\_cos}: Coseno del valor numerico del mes.
    \item \textbf{mes\_sin}: Seno del valor numerico del mes.
    \item \textbf{dia\_mes}: Día del mes (1-31)
    \item \textbf{dia\_mes\_cos}: Coseno del valor numerico del dia del mes.
    \item \textbf{dia\_mes\_sin}: Seno del valor numerico del dia del mes.
    \item \textbf{es\_fin\_de\_semana}: Indicador si es sábado o domingo.
    \item \textbf{es\_feriado}: Indicador si es feriado argentino.
    \item \textbf{estacion}: Estación del año en Hemisferio Sur (categórica).
\end{itemize}

\textbf{Features Climáticas:}
\begin{itemize}
    \item \textbf{temperatura\_promedio}: Categoría de temperatura.
    \item \textbf{llovio}: Indicador si llovió en el día.
    \item \textbf{clima\_adverso}: Indicador de condiciones climáticas adversas.
    \item \textbf{descripcion\_clima}: Descripción textual del clima.
\end{itemize}

\textbf{Features de Lag:}
\begin{itemize}
    \item \textbf{lag\_1\_semana}: Total de ventas para la combinación de producto y turno exactamente una semana atrás.
    \item \textbf{lag\_4\_semanas}: Total de ventas para la combinación de producto y turno exactamente cuatro semanas atrás.
\end{itemize}

\textbf{Rolling Features:} Por cada una de las siguientes ventanas temporales: 1 semana(1w), 2 semanas(2w) y 4 semanas(4w), y combinaciones de producto(p), producto-turno(pt) y producto-día(pdw):
\begin{itemize}
    \item \textbf{roll\_mean\_[ventana]\_[combinacion]}: Promedio móvil de ventas.
    \item \textbf{roll\_std\_[ventana]\_[combinacion]}: Desviación estándar móvil de ventas.
\end{itemize}
...
El modelo se entrenó con \textbf{2000 iteraciones} y una \textbf{tasa de aprendizaje de 0.0347}, lo que permite una convergencia progresiva sin sobreajuste. La \textbf{profundidad de los árboles (5 niveles)} limita la complejidad y mejora la estabilidad del modelo. Se utilizó una \textbf{regularización L2 de 44.95} y una \textbf{temperatura de muestreo de 1.79}, favoreciendo la diversidad de los árboles y reduciendo la varianza.  

El entrenamiento incorpora la técnica de \textit{early stopping} con \textbf{100 iteraciones de paciencia}, finalizando cuando no se observan mejoras significativas. Se empleó la política de crecimiento \textit{SymmetricTree}, propia de CatBoost, que genera árboles balanceados y eficientes.  
